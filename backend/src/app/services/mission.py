...
class MissionService:
    ...
    def __init__(
        ...
        *,
        ...
        self._join_code_verifier = MissionJoinCodeVerifier(repository)  # type: ignore[arg-type]
    ...
    def join(self, request: JoinMissionRequest) -> JoinMissionResponse:
        mission_id = self._join_code_verifier.find_mission_id(request.join_code)
        if mission_id is None:
            raise AppError(
                code="JOIN_CODE_INVALID", message="참여 코드를 확인해 주세요.", status_code=404
            )
