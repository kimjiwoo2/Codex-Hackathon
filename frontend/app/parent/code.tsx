...
export default function JoinCodeScreen() {
  ...
  const joinCode = result?.joinCode ?? '482913';
    ...
    <Screen bottomInset={false}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.codeCard}>
          {joinCode.split('').map((digit, index) => (
            <View key={`${digit}-${index}`} style={styles.codeCell}>
              <Text style={styles.codeDigit}>{digit}</Text>
            </View>
          ))}
        </View>
        <Text style={styles.expiry}>이 코드는 30분 동안 사용할 수 있어요.</Text>
...
const styles = StyleSheet.create({
  ...
  expiry: { color: ICanColors.green, fontSize: 13, marginTop: 16 },
